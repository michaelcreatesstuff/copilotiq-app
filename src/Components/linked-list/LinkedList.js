...
export default class LinkedList {
   ...
   * @param {any} weight
   */
  addFirst(value, weight) {
    const newNode = new Node(value, weight);
   ...
   * @param {any} weight node's weight
   * @returns {Node} newly created node
   */
  addLast(value, weight) {
    const newNode = new Node(value, weight);
   ...
   */
  add(value, weight, position = 0) {
    if (position === 0) {
      // <1>
      return this.addFirst(value, weight);
    }

    if (position === this.size) {
      // <2>
      return this.addLast(value, weight);
    }
    // Adding element in the middle
    const current = this.get(position);
    if (current) {
      const newNode = new Node(value, weight); // <3>
